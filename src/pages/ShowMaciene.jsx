...
const ShowMaciene = () => {
	...
	// Storage の固定ダウンロードトークンから公開URLを決定論的に組み立てる（getDownloadURL の往復を排除）
	const buildDownloadURL = (folder, fileName, token) => {
		if (!fileName || !token) return undefined
			...
			if (demoData.docs.length > 0) {
				...
				for (let i_ = 0; i_ < demoData.docs.length; i_++) {
					...
					const p = (async () => {
						...
						let changed = false
						const innerPromises = fields.map(async (field) => {
							try {
								if (field.includes('Image')) {
									// 固定トークンURLは一度組み立てれば永続有効 → 未取得(undefined)の時だけ構築
									if (in_data[field] === undefined) {
										if (in_data.downloadToken) {
											// 固定トークン方式: URLを決定論的に組み立て
											if (field === 'origineImageURL') {
												// origine は doc 作成時点で必ず存在
												const url = buildDownloadURL(storages[field], in_data.fileName, in_data.downloadToken)
												if (url) { in_data[field] = url; changed = true }
											} else {
												// detected は生成済みか確認してから（未生成なら次回 awaitPaging で再試行）
												try {
													await getMetadata(ref(storage, `${storages[field]}/${in_data.fileName}`))
													const url = buildDownloadURL(storages[field], in_data.fileName, in_data.downloadToken)
													if (url) { in_data[field] = url; changed = true }
												} catch (e) {
													// detected 画像はまだ未生成
											...
											// 旧データ(downloadToken無し)フォールバック: 従来の getDownloadURL
											try {
												const url = await getDownloadURL(ref(storage, `${storages[field]}/${in_data.fileName}`))
												in_data[field] = url
												changed = true
											} catch (e) {
												// 画像はまだ未生成
								...
								} else if (in_data[field] === undefined) {
									// labels: 値が無い時だけ getJson で取得
				...
				let cleanUpdateData = updateData.filter(Boolean);
				console.log(cleanUpdateData)
				if (cleanUpdateData.length > 0) {
					const awaitUpdate = await httpsCallable(function_ini, 'updateStore', { timeout: 550 * 1000 });
					await awaitUpdate(cleanUpdateData).then(async (result) => {
						let mes = result['data']
						console.log(mes)
					})
				}
